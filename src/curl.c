...
static void load_certs_from_dir(const char *dir_path, size_t *ta_capacity, int silent) {
...
static void load_dynamic_certs(int silent) {
    ...
    load_certs_from_dir("/Library/Certificates", &ta_capacity, silent);

    const char *home = getenv("HOME");
    if (home && home[0] && strcmp(home, "/") != 0) {
        char user_certs[256];
        snprintf(user_certs, sizeof(user_certs), "%s/Library/Certificates", home);
        load_certs_from_dir(user_certs, &ta_capacity, silent);
    }

    if (dynamic_TAs_num > 0) {
        if (!silent) printf("Successfully loaded %d dynamic CA certificates\n", (int)dynamic_TAs_num);
    } else {
        if (!silent) printf("No valid certificates loaded.\n");
        free(dynamic_TAs);
        dynamic_TAs = NULL;
    }
}
